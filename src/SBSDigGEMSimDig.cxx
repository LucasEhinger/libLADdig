...
#define fEntranceRef                                                                                                   \
  -1.5 // z position of the copper layer right before the first GEM gas layer,             // relative to the center of
       // the GEM chamber
#define fExitRef                                                                                                       \
  1.5 // z position of the copper layer right before the first GEM gas layer,             // relative to the center of
      // the GEM chamber which introduce additional uncertainty in the lateral direction
...
SBSDigGEMSimDig::SBSDigGEMSimDig() // :
                                   // fGasWion(), fGasDiffusion(), fGasDriftVelocity(), fAvalancheFiducialBand(),
                                   // fAvalancheChargeStatistics(), fGainMean(), fGain0(), fMaxNIon(), fSNormNsigma(),
...
SBSDigGEMSimDig::SBSDigGEMSimDig(int nchambers, double *trigoffset, double zsup_thr, int napv, double *commonmode_array)
    : fZeroSup(zsup_thr) {
...
// ionization Model
//
void SBSDigGEMSimDig::IonModel(TRandom3 *R, const TVector3 &xi, const TVector3 &xo,
...
#define DBG_ION 0
  ...
  if (fRNIon > fMaxNIon) {
#if DBG_ION > 0
    cout << __FUNCTION__ << ": WARNING: too many primary ions " << fRNIon << " limit to " << fMaxNIon << endl;
#endif
    fRNIon = fMaxNIon;
  }
...
#endif
#if DBG_ION > 0
    cout << " x, y = " << ip.X << ", " << ip.Y << " snorm = " << ip.SNorm / fSNormNsigma << " charge " << ip.Charge
         << endl;
...
Short_t SBSDigGEMSimDig::ADCConvert(Double_t val, Double_t off, Double_t gain, Int_t bits) {
  ...
  Short_t dval = static_cast<Short_t>(TMath::Floor((vvv > saturation) ? saturation : vvv));
...
// integration methods
//
void SBSDigGEMSimDig::Integration_semiana(double roangle, double xl, double xr, double yb, double yt, int nx,
                                          double xbw) {
...
void SBSDigGEMSimDig::Integration_fastappx(TRandom3 *R, double roangle, double nions_strip, double xc_hit,
                                           double yc_hit, double dx2_hit, double dy2_hit, double xl, double xr,
                                           double yb, double yt, int nx, double xbw, int ny, double ybw) {
...
// TGEMSBSGEMHit **
void SBSDigGEMSimDig::AvaModel(const int ic, SBSDigGEMDet *gemdet, TRandom3 *R, const TVector3 &xi, const TVector3 &xo,
                               const Double_t t0) {
#if DBG_AVA > 0
  ...
  Double_t gux = (gemdet->GEMPlanes[ic * 2].dX()) / 2. * 1.e3; //+fStripPitch)/2.0*1000.;//(chamber.GetPlane(0).GetStripUpperEdge(chamber.GetPlane(0).GetNStrips()-1)
                                                               //-chamber.GetPlane(0).GetSPitch()/2.0) * 1000.0;
  Double_t guy = (gemdet->GEMPlanes[ic * 2 + 1].dX()) / 2. * 1.e3; //+fStripPitch)/2.0*1000.;//(chamber.GetPlane(1).GetStripUpperEdge(chamber.GetPlane(1).GetNStrips()-1)
  ...
  for (UInt_t ipl = 0; ipl < fNROPlanes; ++ipl) {
...
#if DBG_AVA > 0
    cout << "xr xl yt yb nx ny " << xr << " " << xl << " " << yt << " " << yb << " " << nx << " " << ny << endl;
#endif
    assert(nx > 0 && ny > 0);
    ...
    */
    ...
    for (UInt_t i = 0; i < fRNIon; i++){
      ...
      //cout << " i_chamber " << ic << " iplane " << ipl << endl << " ava x: lower bound " << xl << " center " << frxs
<< " upper bound " << xr << endl << " ava y: lower bound " << yb << " center " << frys << " upper bound " << yt << endl
<< " nstrips " << nstrips << " min bin x " << min_binNb_x << " max bin x " << max_binNb_x << " nx " << nx << " xbw " <<
xbw << endl << " integral " << inte4 << endl;
      ...
      for (Int_t b = 0; b < fEleSamplingPoints; b++) {
        ...
        if (pulse > 0)
          cout << "strip number " << iL + j << ", sampling number " << b << ", t0 = " << t0 << endl
               << "pulse = " << pulse << ", (val - off)/gain = " << (pulse - fADCoffset) / fADCgain
               << ", dadc = " << dadc << endl;
#endif
...
// TGEMSBSGEMHit **
void SBSDigGEMSimDig::AvaModel_2(const int ic, SBSDigGEMDet *gemdet, TRandom3 *R, const TVector3 &xi,
  ...
  for (UInt_t ipl = 0; ipl < fNROPlanes; ++ipl) {
    ...
    Integration_fastappx(R, roangle_mod, NionsStrip[ipl], xc_hit, yc_hit, dx2_hit, dy2_hit, xl, xr, yb, yt, nx, xbw, ny,
                         ybw);
    //}
...
Int_t SBSDigGEMSimDig::Digitize(SBSDigGEMDet *gemdet, TRandom3 *R, bool bkgdonly) //,
// gmn_tree* T)
{
  ...
  for (size_t ih = 0; ih < gemdet->fGEMhits.size(); ih++) {
    ...
    TVector3 vv1(gemdet->fGEMhits[ih].xin * 1.e3, gemdet->fGEMhits[ih].yin * 1.e3, gemdet->fGEMhits[ih].zin * 1.e3);
    TVector3 vv2(gemdet->fGEMhits[ih].xout * 1.e3, gemdet->fGEMhits[ih].yout * 1.e3, gemdet->fGEMhits[ih].zout * 1.e3);
    /*
    if(is_background){
    ...
    */
    ...
    // fRSMax = TMath::Sqrt(2.*fGasDiffusion*TMath::Abs(fRoutZ-fEntranceRef)/fGasDriftVelocity);//maximum avalanche
    // spread =  maximum time = maximum distance fRTime0 = TMath::Abs(fRoutZ-fExitRef)/fGasDriftVelocity;//minimum time
    // = minimum distance cout << " Digitize " << gemdet->fGEMhits[ih].edep << " " << fRNIon << " " << fRSMax << " " <<
    // fRTime0 << endl;
    // }else{
...
#if DBG_AVA > 0
    if (time_zero > 200.0)
      cout << "time_zero " << time_zero << "; evt time " << event_time << "; hit time " << gemdet->fGEMhits[ih].t
           << "; drift time " << fRTime0 * 1e9 << endl;
#endif
    if (fRNIon > 0) {
      // cout << "AvaModel..." << endl;
      ...
      std::chrono::time_point<std::chrono::steady_clock> fStart2 = std::chrono::steady_clock::now();
      if (is_background) {
...
inline static Double_t IntegralY(Double_t *a, Int_t ix, Int_t nx, Int_t ny) {
...
void SBSDigGEMSimDig::CheckOut(SBSDigGEMDet *gemdet, const int uniqueid, TRandom3 *R, g4sbs_tree *T, bool sigonly)
// gmn_tree* T)
{
      ...
      if (gemdet->GEMPlanes[i].GetADCSum(j) > 0) {
        ...
        if (!sigonly) {
          for (int k = 0; k < fNSamples; k++) {
            // cout << i << " " << j << " " << k << " " << gemdet->GEMPlanes[i].GetADC(j, k) << " " <<
            // gemdet->GEMPlanes[i].GetADCSum(j) << " = = > "; int ped = TMath::Nint(R->Gaus(commonmode,
            // fPulseNoiseSigma)); if(gemdet->GEMPlanes[i].GetADC(j, k)<0 || gemdet->GEMPlanes[i].GetADC(j, k)>4096)cout
            // << i << " " << j << " " << k << " " << gemdet->GEMPlanes[i].GetADC(j, k) << " " << ped << " " <<
